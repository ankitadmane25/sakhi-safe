...
function sendAlerts(locationStr) {
    ...
    // ==========================================
    // SHARE VIA USER'S OWN NUMBER (NATIVE SMS)
    // ==========================================
    showToast(`Opening SMS app to alert ${contacts.length} contacts...`, "success");

    // Format contacts for the SMS URI (comma separated)
    const phoneList = contacts.join(',');
    // iOS uses '&' for the body parameter, Android/Others use '?'
    const isIOS = /iPad|iPhone|iPod/.test(navigator.userAgent) && !window.MSStream;
    const separator = isIOS ? '&' : '?';

    const smsLink = `sms:${phoneList}${separator}body=${encodeURIComponent(message)}`;

    // Automatically trigger the SMS app with pre-filled details
    // Uses the user's own cellular connection and phone number
...
window.addEventListener('devicemotion', (event) => {
    if (!currentUser || isSosActive) return;

    // iOS permission check
    if (typeof DeviceMotionEvent.requestPermission === 'function' && !isShakeEnabled) {
        return;
    }
    ...
    if ((currTime - lastUpdate) > 100) {
        ...
        if (lastX !== null) {
            ...
            if (speed > 1200) { // slightly lower for better detection
                ...
                if (shakeCount >= 2) { // faster trigger
                    showToast("Shake detected! Sending SOS...", "error");
                    triggerSOS();
                    shakeCount = 0;
                }
            }
